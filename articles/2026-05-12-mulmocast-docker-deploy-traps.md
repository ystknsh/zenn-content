...
字幕焼き込みモード `mulmo movie -c <lang>` は内部で Puppeteer (Chrome) を起動して HTML をレンダリングします。`node:slim` には Chrome が一切入っていないため、以下の 4 つの設定が必要です。
...
**なぜ `--no-sandbox` でも安全か**: 字幕生成は自前の HTML を headless でレンダリングして画像化するだけで、外部の悪意あるコードを評価しません。Docker コンテナ自体が隔離環境なので、Chrome のサンドボックスを重ねる実利益は薄いです（CI 環境で `--no-sandbox` が慣例なのと同じ理由です）。
...
TypeScript のサーバーは `tsc` で `dist/` にコンパイルされますが、**JSON / mp3 / png は当然コピーされません**。`mulmo audio` / `mulmo movie` は実行時にこれらを相対パスで参照するので、コピーが漏れると次のようなエラーになります。
...
- ❌ Replicate API 側のタイムアウト → mulmo が `await` で待っている限り Replicate は何分でも待つ
- ❌ ALB の idle timeout → HTTP レイヤなので生成プロセスとは別軸
...
- **`exited with code null` + 途中ログで停止 = `child_process.spawn` の timeout による中断**。レイヤを取り違えると Express や ALB のタイムアウトを延々と触ってしまいます。
- **外部生成 API を直列で呼ぶ系は 30 分が現実的な下限**。API 側のキュー待ちは「平均」では語れない（P99 で 3 倍以上ぶれる）です。
...
t3.small / t3.medium クラスのインスタンスは **メモリが 2〜4GB しかなく、Amazon Linux 2 のデフォルトでは swap が一切設定されていません**。Node.js のビルド + Chromium 起動 + ffmpeg + 外部 API レスポンスのバッファが同時に走るとメモリが不足し、OOM Killer によってコンテナが停止することがあります。
...
ポイント: **Docker で headless Chrome + Node を動かすなら swap は事実上必須**。本番運用に入る前に必ず確認します。ただし swap は遅いので、頻繁に使われるなら本来はインスタンスサイズを上げるべきサインです。
...
ポイント: **「ローカルは通って本番だけ失敗する Docker build」の最初の被疑者はディスク容量**。次が CPU アーキテクチャ、その次がプロキシ・DNS・rate limit です。
