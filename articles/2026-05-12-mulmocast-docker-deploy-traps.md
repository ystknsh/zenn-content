...
MulmoCast CLI (`mulmo audio` / `mulmo movie` コマンド) を Express サーバーから `child_process.spawn` で呼び出し、Cloudflare Pages + EC2 Docker で本番運用するときの推奨構成と、設計上の注意点をまとめました。

設定すべき項目は Puppeteer・アセットコピー・ボリュームマウント・タイムアウト・EC2 ホストのリソースなど、複数レイヤーに分散しています。最初から正解の構成で始めるための指針として参考にしてください。
...
- ❌ Replicate API 側のタイムアウト → このケースでは Replicate 側の制限時間に達する前に `spawn` の timeout が先に発火している
