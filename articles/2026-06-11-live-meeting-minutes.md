...
![前回 TODO 進捗を各担当者が会議前に記入した例](/images/2026-06-11-live-meeting-minutes/meeting-doc-prev-todo.png)
...
実際に使っている Google Docs のサンプルを、**会議前（左）** と **会議後（右）** で並べました。左（会議前）は当回（06/10）がまだアジェンダだけ。右（会議後）は各議題の下に `<...>` で決定・TODO・DECIDED が追記されています（会議中にその場で書いた部分を、この図とリンク先の Doc では分かりやすさのため赤字にしています。実際の運用では後述のコラムの通り、色ではなく文字で表現します）。
...
サンプル（架空の社内プロジェクト「展示会出展準備」）の読み方は次の通りです。

- **平文部分** は、ファシリテーターが **会議前に書いて配布** した内容（アジェンダ、各議題の `決めたいこと` ＋ `選択肢/案`）
- **前回 TODO 進捗** は、**会議前に各担当者が記入** しておく（→ 2.3）
- **`<...>` 部分** は、**会議中にその場で追記** した内容（決定・TODO・補足）。TODO はその場で `TODO:` プレフィックスを付けて該当議題の下に書き足す
