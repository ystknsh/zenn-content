...
2024年11月23日(土)、東京・大崎にあるファインディ株式会社様のイベントスペースにて「Fullstack AI Dev & Raycast Summit feat. Satoshi Nakajima」を開催いたしました。イベントレポートのメイン記事は[こちら](https://zenn.dev/raycast_jp/articles/2024-11-23-fullstack-ai-dev-n-raycast-summit-main)です。
本記事ではトークセッション1 および スポンサートークについて紹介いたします。

# 有本勇氏 / GraphAI: Full-Stack TypeScript Tool for AI Applications
:::message
**有本勇氏 プロフィール**  
...
# 上野彰大氏 / LLMマルチエージェントアプリケーションの設計のコツ
:::message
**上野彰大氏 プロフィール**  
...
## 登壇概要 (登壇資料より抜粋に加えて、Claude による要約)
...
### システム設計の特徴
システムはLangChain、LangSmith、LangGraphといったエコシステムを活用し、以下のような段階的な処理フローを実装しています：
...
# BASE株式会社様 / オフラインでもTypeScriptから使えるAI ~ Chromeに搭載されるGemini Nanoとは ~
:::message
**BASE株式会社様 会社情報**  
■ BASE BANKチームのご紹介
...
### Chrome組み込みAI(SLM)の可能性
Small Language Model(SLM)は、従来のLarge Language Model(LLM)と比較して小規模なモデルです。LLMが大量のコンピューティングリソースを必要とするのに対し、SLMは必要リソースが少なく、個人のデバイスに実装可能という特徴があります。

### Chrome搭載のSLMの特徴
GoogleはChrome向けにデバイス上でSLMを実行する機能があります。この機能には以下の利点があります：

- オフライン実行が可能
- 利用制限がない
- セキュアな実行環境の提供
- APIキー不要でTypeScriptから直接利用可能

### 実装状況と制約
現在は実験的機能として提供されており、以下の制約があります：
- ストレージ要件22GB以上の空き容量が必要
