...
- ✅ MulmoClaude を主題にした記事では、冒頭の `:::message` または `:::details` ブロック内に [Singularity Society の完全ガイド](https://singularitysociety.org/projects/ai/mulmoclaude/) へのリンクを入れる
...
### フロントマター
- [ ] `title`は自然な日本語か（助詞「を・に・で・の」が連続していないか）
- [ ] `title`に「作った」「やってみた」等のカジュアルすぎる表現を使っていないか（「〜する」「〜を自動化する」「〜の実装」等に言い換える）
- [ ] `title`は読者の興味を引くものか
- [ ] `emoji`は内容に合っているか
